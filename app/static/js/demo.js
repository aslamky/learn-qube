var OV,OV2;           // OpenVidu object to initialize a session
var session, session2;        // Session object where the user will connect
var publisher, publisher2;        // Publisher object which the user will publish
...
function joinRoom(sessionId) {
  ...
  session = OV.initSession(
      // "wss://" + location.hostname + ":8443/" + sessionId + "?secret=openvidu"
     OPENVIDU_URL + sessionId + "?secret=openvidu"
  );
...
session2.on('streamCreated', function (event) { 
    // Subscribe to the Stream to receive it. HTML video will be appended to element with 'subscriber' id
    var subscriber2 = session2.subscribe(event.stream, 'videos');
    // When the new video is added to DOM, update the page layout to fit one more participant
...
session2.connect(userId2, function (error) {
    ...
    if (!error) {
      ...
      publisher2 = OV2.initPublisher('publisher', {
        audio: false,        // Whether you want to transmit audio or not
        video: true,        // Whether you want to transmit video or not
        audioActive: false,  // Whether you want to start the publishing with your audio unmuted or muted
        videoActive: true,  // Whether you want to start the publishing with your video enabled or disabled
        quality: 'MEDIUM',  // The quality of your video ('LOW', 'MEDIUM', 'HIGH')
